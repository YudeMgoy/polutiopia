...
namespace Hypertonic.GridPlacement
{
    ...
    public class GridManager : MonoBehaviour
    {
        ...
        private AsyncHelpers _asyncHelpers;
        ...
        private void Awake()
        {
            ...
            if (!TryGetComponent(out _asyncHelpers))
                _asyncHelpers = gameObject.AddComponent<AsyncHelpers>();
        ...
        /// <returns> If the grid was successfully populated with the grid data </returns>
        public async Task<bool> PopulateWithGridData(GridData gridData, bool clearGrid = true)
        {
            if (clearGrid)
            {
                ClearGrid();
                await _asyncHelpers.WaitForEndOfFrame();
            }
            ...
            {
                bool success = await AddObjectToGridByCellAsync(gridObjectPositionData[i].GridObject, gridObjectPositionData[i].GridCellIndex, gridObjectPositionData[i].ObjectAlignment);
        ...
        public async Task<bool> AddObjectToGridByCellAsync(GameObject gridObject, Vector2Int gridCellIndex, ObjectAlignment objectAlignment)
        {
            ...
            await _asyncHelpers.WaitForFixedUpdate();
