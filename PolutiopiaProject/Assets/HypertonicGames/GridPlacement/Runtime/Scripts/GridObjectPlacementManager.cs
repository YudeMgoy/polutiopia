...
namespace Hypertonic.GridPlacement
{
    public class GridObjectPlacementManager : MonoBehaviour
    {
        ...
        public void Setup(GridManager gridManager, GridSettings gridSettings, GameObject objectToPlace)
        {
            ...
            _objectPlacementContainer.transform.rotation = Quaternion.identity;
            ...
            ObjectToPlace.transform.SetParent(_objectPlacementContainer.transform, false);
            _objectPlacementContainer.transform.rotation = Quaternion.Euler(new Vector3(0, _gridManager.RuntimeGridRotation, 0));

            UpdateSnapAlignment(_gridSettings.DefaultAlignment);
            _originalObjectLayer = objectToPlace.layer;

            _objectPlacementContainer.transform.position = objectToPlaceCurrentPosition;
        ...
        {
            _objectPlacementContainer.transform.position -= CustomPositionOffset;
        ...
        {
            _objectPlacementContainer.transform.position += positionDifference;
        }

        public void HandleGridRotationUpdated(float rotationDifference)
        {
            float currentRot = Mathf.RoundToInt(_objectPlacementContainer.transform.rotation.eulerAngles.y * 10000) / 10000;
            float newRot = Mathf.RoundToInt((currentRot + rotationDifference) * 10000) / 10000;
            float diff = newRot - currentRot;

            _objectPlacementContainer.transform.RotateAround(_gridManager.GridTransformGO.transform.position, Vector3.up, diff);
        }
        ...
        {
            _objectPlacementContainer.transform.position -= Quaternion.Euler(0, _gridManager.RuntimeGridRotation, 0) * SnapAlignmentOffset;
        ...
        public void UpdateObjectPosition(Vector3 worldPos)
        {
            ...
            _objectPlacementContainer.transform.position = Quaternion.Euler(0, _gridManager.RuntimeGridRotation, 0) * (CenterOfGridArea + PositionOffset + SnapAlignmentOffset + CustomPositionOffset);
        }
        ...
        public Vector3 GetCurrentPositionOfObject() => _objectPlacementContainer.transform.position - PositionOffset;
